        ...
        public object DrawAndGetNewValue(Type type, string fieldName, object value, Entity entity, int index, IComponent component) {
            ...
            if (array.Rank == 1) {
                ...
                for (int i = 0; i < array.GetLength(0); i++) {
                    EditorGUILayout.BeginHorizontal();
                    EntityEditor.DrawAndSetElement(elementType, fieldName + "[" + i + "]", array.GetValue(i),
                ...
                for (int i = 0; i < array.GetLength(0); i++) {
                    for (int j = 0; j < array.GetLength(1); j++) {
                        EntityEditor.DrawAndSetElement(elementType, fieldName + "[" + i + ", " + j + "]", array.GetValue(i, j),
                ...
                for (int i = 0; i < array.GetLength(0); i++) {
                    for (int j = 0; j < array.GetLength(1); j++) {
                        for (int k = 0; k < array.GetLength(2); k++) {
                            EntityEditor.DrawAndSetElement(elementType, fieldName + "[" + i + ", " + j + " ," + k + "]", array.GetValue(i, j, k),
