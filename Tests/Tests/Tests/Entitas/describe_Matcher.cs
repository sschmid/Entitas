...
class describe_Matcher : nspec {
    ...
    static void assertIndicesContain(int[] indices, params int[] expectedIndices) {
        indices.Length.ShouldBe(expectedIndices.Length);
        for (int i = 0; i < expectedIndices.Length; i++) {
            indices[i].ShouldBe(expectedIndices[i]);
        }
    }
    ...
    void when_creating_matcher() {
        ...
        context["allOf"] = () => {
            ...
            it["caches indices"] = () => m.indices.ShouldBeSameAs(m.indices);
            it["doesn't match"] = () => m.Matches(eA).ShouldBeFalse();
            it["matches"] = () => {
                m.Matches(eAB).ShouldBeTrue();
                m.Matches(eABC).ShouldBeTrue();
            };
            ...
            it["uses componentNames when merged matcher ToString"] = () => {
                ...
                var mergedMatcher = Matcher<TestEntity>.AllOf(m1, m2, m3);
                mergedMatcher.ToString().ShouldBe("AllOf(m_1, m_2, m_3)");
            };
        };
        ...
        context["anyOf"] = () => {
            ...
            it["caches indices"] = () => m.indices.ShouldBeSameAs(m.indices);
            it["doesn't match"] = () => m.Matches(eC).ShouldBeFalse();
            it["matches"] = () => {
                m.Matches(eA).ShouldBeTrue();
                m.Matches(eB).ShouldBeTrue();
                m.Matches(eABC).ShouldBeTrue();
            };
            ...
            it["can ToString"] = () => m.ToString().ShouldBe("AnyOf(1, 2)");
        };
        ...
        context["allOf.noneOf"] = () => {
            ...
            it["caches indices"] = () => m.indices.ShouldBeSameAs(m.indices);
            it["doesn't match"] = () => m.Matches(eABC).ShouldBeFalse();
            it["matches"] = () => m.Matches(eAB).ShouldBeTrue();
            ...
            it["can ToString"] = () => m.ToString().ShouldBe("AllOf(1, 2).NoneOf(3, 4)");

            it["uses componentNames when componentNames set"] = () => {
                var matcher = (Matcher<TestEntity>)m;
                matcher.componentNames = new [] { "one", "two", "three", "four", "five" };
                matcher.ToString().ShouldBe("AllOf(two, three).NoneOf(four, five)");
            };
        };
        ...
        context["anyOf.noneOf"] = () => {
            ...
            it["caches indices"] = () => m.indices.ShouldBeSameAs(m.indices);
            it["doesn't match"] = () => m.Matches(eABC).ShouldBeFalse();
            it["matches"] = () => m.Matches(eA).ShouldBeTrue();
            it["matches"] = () => m.Matches(eB).ShouldBeTrue();
            ...
            it["can ToString"] = () => m.ToString().ShouldBe("AnyOf(1, 2).NoneOf(3, 4)");
        };
        ...
        context["allOf.anyOf"] = () => {
            ...
            it["caches indices"] = () => m.indices.ShouldBeSameAs(m.indices);
            it["doesn't match"] = () => m.Matches(eAB).ShouldBeFalse();
            it["matches"] = () => m.Matches(eABC).ShouldBeTrue();
            ...
            it["can ToString"] = () => m.ToString().ShouldBe("AllOf(1, 2).AnyOf(3, 4)");
        };
        ...
        context["indices cache"] = () => {
            ...
            it["updates cache when calling AnyOf"] = () => {
                ...
                m.indices.ShouldNotBeSameAs(cache);
            };
        ...
        context["equals"] = () => {
            ...
            it["updates hash when changed with anyOf"] = () => {
                ...
                m1.AnyOf(42).GetHashCode().ShouldNotBe(hash);
            };
            ...
            it["updates hash when changed with noneOf"] = () => {
                ...
                m1.NoneOf(42).GetHashCode().ShouldNotBe(hash);
            };
            ...
            it["equals merged matcher"] = () => {
                ...
                mergedMatcher.Equals(m3).ShouldBeTrue();
                mergedMatcher.GetHashCode().ShouldBe(m3.GetHashCode());
            };
            ...
            it["doesn't equal different AllOfMatcher"] = () => {
                ...
                m1.GetHashCode().ShouldNotBe(m2.GetHashCode());
            };
            ...
            it["doesn't equal differnt type matchers with same indices"] = () => {
                ...
                m3.Equals(m4).ShouldBeFalse();
                m3.GetHashCode().ShouldNotBe(m4.GetHashCode());
            };
            ...
            it["equals compound matcher"] = () => {
                ...
                mX.Equals(mY).ShouldBeTrue();
                mX.GetHashCode().ShouldBe(mY.GetHashCode());
            };
        };
    }
