    ...
    public class ComponentBlueprint
    {
        ...
        public IComponent CreateComponent(IEntity entity)
        {
            if (_type == null)
            {
                ...
                if (_type == null)
                {
                    throw new ComponentBlueprintException(
                        $"Type '{fullTypeName}' doesn't exist in any assembly!",
                ...
                {
                    throw new ComponentBlueprintException(
                        $"Type '{fullTypeName}' doesn't implement IComponent!",
                        $"{nameof(ComponentBlueprint)} only supports IComponent."
                    );
                }
            }
            ...
            for (var i = 0; i < members.Length; i++)
            {
                ...
                if (_componentMembers.TryGetValue(member.name, out var memberInfo))
                {
                ...
                {
                    Console.WriteLine(
                        $"Could not find member '{member.name}' in type '{_type.FullName}'!\nOnly non-static public members are supported."
                    );
                }
            }
